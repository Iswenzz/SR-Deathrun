...
main()
{
 maps\mp\_load::main();
	...
	level.music=[];
    level.music[0]["song"]    ="Au5 - Snowblind";
    level.music[0]["alias"]    ="song1";
    level.music[1]["song"]    ="Skrux - Universe";
    level.music[1]["alias"]    ="song2";
    level.music[2]["song"]    ="AVERDECK - Time";
    level.music[2]["alias"]    ="song3";
    level.music[3]["song"]    ="Illenium - Needed You";
    level.music[3]["alias"]    ="song4";
    level.music[4]["song"]    ="Laxxtrak - Sorry x Calling";
    level.music[4]["alias"]    ="song5";
	level.music[5]["song"]    ="Ben Walter - New Dawn";
...
musicbox()
{
    ...
    trig setHintString("Press ^5&&1^7 to select song");
...
musicmenu()
{
    ...
    self.hud_music[i] setShader( "roccialiena2d", 320, 160 );
    i++;
    self.hud_music[i] = braxi\_mod::addTextHud( self, 167, 204, 0.6, "left", "top", 1.4 );
    self.hud_music[i].sort = 880;
    self.hud_music[i] setShader( "white", 306, 20 );
    self.hud_music[i].color=(0,0.7,0.8);

    i++;
    self.hud_music[i] = braxi\_mod::addTextHud( self, 210, 204, 0.93, "left", "top", 1.8 );
    self.hud_music[i].sort = 884;
    self.hud_music[i] setText("Lava World Winter Music Menu");
    self.hud_music[i].glowalpha=1;
    if(isdefined(level.randomcolor))
        self.hud_music[i].glowcolor=level.randomcolor;
    else
        self.hud_music[i].glowcolor=(0,0.7,0.8);
    i++;
...
dropdopop = getEnt("dropdopop", "targetname");
	...
	wait 3;
	level.topcredits setText("^5Every"+level.timetext+"seconds normal platforms will disappear");
	level.topcredits fadeovertime(1);
	wait 5;
	level.topcredits setText("^5Cyan ^3platforms ^8won't");
	level.topcredits fadeovertime(1);
	wait 5;
    ...
    for(;;)
    {
		...
		level.topcredits setText("^5Floor dropping in: ^2" + level.timer);
		 ...
		 while (level.timer < 4 && level.timer > 0)
		 {
	      ...
	      wait 1;
		  level.timer--;
		  level.topcredits setText("^5Floor dropping in: ^1" + level.timer);
		  timersound = true;
		  }
		  else
		  {
		...
		if (level.timer == 0)
		{
		level.topcredits setText("^5Floor dropped, please wait...");
		musicStop(0);
		musicPlay ("susp");
...
{
ambientPlay("song1");
iPrintln("^2>>^1Now playing: ^5Au5 - Snowblind^2<<");
}

if (x==1)
{
ambientPlay("song2");
iPrintln("^2>>^1Now playing: ^5Skrux - Universe^2<<");
}

if (x==2)
{
ambientPlay("song3");
iPrintln("^2>>^1Now playing: ^5AVERDECK - Time^2<<");
}

if (x==3)
{
ambientPlay("song4");
iPrintln("^2>>^1Now playing: ^5Illenium - Needed You^2<<");
}

if (x==4)
{
ambientPlay("song5");
iPrintln("^2>>^1Now playing: ^5Laxxtrak - Sorry x Calling^2<<");
}

if (x==5)
{
ambientPlay("song6");
iPrintln("^2>>^1Now playing: ^5Ben Walter - New Dawn^2<<");
}
...
{
wait 10;
iprintlnBold("^5Lava World Winter");
wait 5;
...
if (x==1)
{
iPrintln("^5You will have less time to find a safe zone if the speed is higher than 190!");
}

if (x==2)
{
iPrintln("^7There is ^51 ^7Secret on this Map! Try finding it ;)");
}

if (x==3)
{
iPrintln("^5From an original idea of ^0Dark^5STEP");
}

if (x==4)
{
iPrintln("^5Steam: ^5CAR ^0DarkSTEP");
}
...
if (x==6)
{
iPrintln("^5Feel free to join Amnesia's Deathrun!");
}
...
if (x==8)
{
iPrintln("^5Merry Christmas peasants ;)");
}
...
{
	iPrintLnBold ("^5Always keep an eye on the timer up here...");
}
...
trig setHintString("Press F to throw players in ice!");
trig waittill("trigger", player);
trig delete();
...
safe()
{
trigger = getent("trig_safe","targetname");
...
thread removeColorFromString(gametag);
	if ( (isSubStr( toLower(gametag), toLower("CAR")) ) || (isSubStr( toLower(gametag), toLower("Rex")) ) || gametag == "Fish Da Rekter" || gametag == "Death" || gametag == "boss'Death" || gametag == "eBc|Death" || gametag == "Hazard" || gametag == "ESP'Hazard<3" || gametag == "LaRamz")
...
supershit()
{
	...
	// 	player iPrintLnBold( "^5Sorry but ^6xM members are ^1not^5 allowed to play my maps" );
	// 	wait 1.5;
...
trigger sethintstring ("^1Press ^5F ^1to turn on lasers!");
trigger waittill ("trigger", player);
trigger delete();
...
gay()
{
	...
	trig setHintString ("^5Press F to kill everyone");
	trig waittill ("trigger", who);
	hud_Screen = NewClientHudElem(who);
    hud_Screen.horzalign = "fullscreen";
	hud_Screen.vertalign = "fullscreen";
...
bounce1s()
{
	...
	for (;;)
	{
		...
		player setOrigin(o.origin);
		player setPlayerAngles(o.angles);
		player iPrintLnBold("^5NAB SAVER");
	}
}
...
secretosexit()
{
	...
	for (;;)
	{
		...
		player setOrigin(o.origin);
		player setPlayerAngles(o.angles);
		iPrintLn(player.name+" ^5finished the secret!");
		player braxi\_rank::giveRankXp( "", 1000);
		wait .1;
...
secretostep1()
{
	...
	wait 15;
	trig waittill("trigger", player);
	player iPrintLnBold("^5Something happened...");
	iPrintLn("^11^0/^52");
	wall delete();
	trig delete();
}
...
secretostep2()
{
	...
	arabclip rotateYaw(-2160, 2);
	wait 1;
	arab moveZ(-1000, 1);
	arablock moveZ(-1000, 1);
	arabclip moveZ(-1000, 1);
	musicPlay("susp");
	wait 1;
...
secret_hud()
{
    ...
    self.secretTimer.label = &"^5Time left: ^2&&1";
...
snow()
{
	o = getEntArray("snow","targetname");
	for(;;)
	{
	for(i = 0; i < o.size; i++)
	{
		playFx(level.snow, o[i].origin );
    }
	wait 0.3;
	}
}
...
boom()
{
	trig = getEnt("trig_boom", "targetname");
	for(;;)
	{
		trig waittill("trigger", player);
		...
		//player setModel("darksamus");
		iPrintln("^2>>^1Now playing: ^5Tiesto - BOOM^2<<");
		return;
		}
	}
}