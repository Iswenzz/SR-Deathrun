...
//TODO: 
//Fix secrets timers so it prints the correct time
//Fix the "GetActivator" function so it actually gets when the activator there and alive.
...

	{
		...
		if(i == 0)
		{
			if( isDefined( level.activ ))
				level.activ playLocalSound("song1");
		}
		else if(i == 1)
		{
			if( isDefined( level.activ ))
				level.activ playLocalSound("song2");
		}
	}
}
...
darkstar9t = getEnt("dark_star_1_secret","targetname");
	...
	iPrintLnBold( player.name + " ^1Found the ^0Dark Star");
...
h1tp = getEnt("hardsecret_finish_tp","targetname");
	...
	player braxi\_rank::giveRankXp("", 100);
	iprintlnbold( player.name + " ^3Finished ^1HARD ^7CoDJumper way!!!!" );
	iprintln("this meant to be a secret but everyone knows it");
...
endEasySecretTimer()
{
	...
	time1 = int((getTime() - self.timer1StartTime) / 1000);
...
easySecretTimer()
{
	...
	self iPrintLnBold("You ^7have ^360 ^7seconds to finish!");
...
endHardSecretTimer()
{
	...
	time2 = int((getTime() - self.timer2StartTime) / 1000);
	...
	iPrintlnBold( self.name + " finished ^1HARD ^7not so secret slide in ^1" + time2 );

	if( time2 < int(self.pers["time2"]) )
...
hardSecretTimer()
{
	...
	wait 0.05;
	self playSound("warp1");
	self iPrintLnBold("^1You have ^330 ^7seconds to ^1finish^7!");
...
deathsecretslide()
{
	...
	if(isDefined( self.hud_easysecret ))
		self.hud_easysecret destroy();
	if(isDefined( self.hud_easysecret1 ))
		self.hud_easysecret1 destroy();
	if(isDefined( self.hud_hardsecret ))
		self.hud_hardsecret destroy();
	if(isDefined( self.hud_hardsecret1 ))
...
star1 = getEnt("star_1","targetname");
	...
	player iPrintLnBold("You ^7won ^350 XP ^7!");
...
coin1trig = getEnt("coin_1_trig","targetname");
	...
	player iPrintLnBold("You ^7won ^315 XP ^7!");
	wait 0.2;
	}
}
...
trapmaskass2()
{
	...
	maskas1ori rotateYaw(720, 5, 0.5, 0.5);

	maskas1ori moveZ(250, 2, 0.5, 0.5);
	wait 3;
	maskas1ori moveZ(-250, 2, 0.5, 0.5);
	wait 3;
}
...
{
	mrball1 = getEnt("movingball1", "targetname");
	mrball2 = getEnt("movingball2", "targetname");
	while( 1 )
	{
		mrball1 rotatepitch(360, 1.5);
		mrball2 rotatepitch(360, 1.5);
		wait 1.5;
	}
}
...
//	level.player allowProne(false);
	...
	while(isalive(self) && self istouching(trigger) && (self.progresstime < level.drown_watertime))
	{		
		d ++;
		f ++;
	...
	if(isalive(self) && self istouching(trigger) && (self.progresstime >= level.drown_watertime))
	{
...
{ 
self common_scripts\utility::waittill_any("death","disconnect");
iPrintlnBold("^2"+self.name+" ^7has died"); 
iPrintlnBold("Selection Room is now open!!"); 
}
...
sniperroom() //Sniper Room
{
	...
	for(;;)
	{
		...
		thread createhud(player.name + " ^3selected ^7sniper room!");
		wait 5;
		...
		if( isDefined( level.activ ))
		{
			level.activ iprintlnbold("^5FIGHT^7!!!");
...
weaponsroom() //weapons room :)
{
	...
	for(;;)
	{
		...
		player thread waitdead();
		thread createhud(player.name + " ^3Selected ^7Weapons Room!!!");
		if( isDefined( level.activ ))
		{
			level.activ thread songroom();
			level.activ freezeControls(true);
...
bounceroom() //bounce room :p
{
	...
	for(;;)
	{
		...
		player setPlayerAngles(jumper.angles);
		player giveweapon("knife_mp");
		thread createhud(player.name + " ^2Selected Bounce Room!!!");
		wait 5;
		iprintlnbold("GO!!!");
		if( isDefined( level.activ ))
		{
...
runroom() //Speedrun room
{
	...
	for(;;)
	{
		...
		player setPlayerAngles(jumper.angles);
		player giveweapon("deserteaglegold_mp");
		thread createhud(player.name + " Selected speed Run Room!!!");
		wait 5;
		iprintlnbold("GO!!!");
		player freezeControls(false);
		if( isDefined( level.activ ))
...
kniferoom() //Knife room lel
{
	...
	for(;;)
	{
		...
		thread createhud(player.name + " Selected Knife room.");
		wait 5;
		player freezeControls(false);
