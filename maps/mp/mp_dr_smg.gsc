...
startdoor(){
	...
	wait 1;
	iprintlnbold("^8Startdoor Opening in");
	iprintlnbold("^83");
	wait 1;
	iprintlnbold("^62");
	wait 1;
...
trap2(){
	...
	origin = ent.origin;
	while(1){
		playfx(level.fx_steam, origins[0].origin, (-1, 1, 0));
		playfx(level.fx_steam, origins[1].origin, (-1, 1, 0));
		ent moveTo((382,1996, 10668), 0.5);
		wait 1.5;
		ent moveTo(origin, 0.1);
		wait 3;
	}
}
...
trap4(){
	...
	rock1 thread roll_rock1(rock1_origin);
	rock2 thread roll_rock2(rock2_origin);
	rock3 thread roll_rock3(rock3_origin);

	wait 6;

	while(1){
		rock1 roll_rock1(rock1_origin);
		wait 2;
		rock2 roll_rock2(rock2_origin);
		wait 2;
		rock3 roll_rock3(rock3_origin);
		wait 2;
	}
}
...
rock_drop(){
	...
	moveTrig maps\mp\_utility::triggerOff();

	trigger = getent("drop_rocks", "targetname");
	trigger setHintString("Press ^2[Use] ^7to Lower Rock");
	trigger waittill("trigger");
	trigger delete();

	moveTrig maps\mp\_utility::triggerOn();

	ent1 movez(-220,1);
	coll1 movez(-220,1);

	level notify("disable_wall_jump_1");
...
stage2_rock_drop_2(){
	...
	trigger setHintString("Press ^2[Use] ^7to drop the platform");
	trigger waittill("trigger");
	trigger delete();
	ladder delete();
